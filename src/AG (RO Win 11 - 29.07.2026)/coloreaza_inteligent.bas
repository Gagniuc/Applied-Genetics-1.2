Attribute VB_Name = "UNIVERSAL_EDITARE"



Option Explicit
'API declarations
'type used to calculate current character under mouse
Public Type POINTL
    X As Long
    Y As Long
End Type
...
Public COD_PASAT_FUNCTIE As String
...
Public Sub HighLightSelection(mForm As Form, mRTF As RichTextBox, mHighLightColor As Long, Optional DontLock As Boolean)
    ...
    Dim RepairCtbl As Boolean
    Dim OldCol As Long
    ' Fix: susu era declarata la nivel de modul si nu se reseta niciodata
    ' intre apeluri -- dupa destule apeluri depasea deja 400 din apeluri
    ' anterioare, bucla de mai jos iesea imediat fara sa curete highlight-ul
    ' vechi, iar tag-urile \highlight ramase se acumulau in tot documentul RTF,
    ' incetinind progresiv fiecare apel urmator.
    Dim susu As Long
    ...
    If Z <> 0 Then
        ...
        Do
            ...
            On Error Resume Next '-eu
            susu = susu + 1
            If susu > 400 Then Exit Do
            Z = InStr(1, SelectedText, "\highlight")
            If Z = 0 Then Exit Do
        Loop
