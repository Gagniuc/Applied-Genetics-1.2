...
'*************************************************************************
'* MorphDisplay v1.01 - Ownerdrawn digital display user control.         *
'* Written January, 2006, by Matthew R. Usner for Planet Source Code.    *
'* Last update 16 May 2006 - HH:MM A/P time display capability.          *
'*************************************************************************
...
'  0-9, unlit segment, minus sign, hex A-F, and P.  19 patterns total.
Private DisplayPattern(0 To 18) As String
...
'  The center segment is #7.  A "1" means the segment is lit.
   ...
   DisplayPattern(12) = "1110111"   ' Hex "A" (also for time display).
   ...
   DisplayPattern(18) = "1100111"   ' "P" (for time display).
...
'     the pattern index for the appropriate hex value A-F.
   ...
   ElseIf strDigit = "P" Then
      GetDisplayPatternIndex = 18
   Else
         ...
         Select Case m_SegmentFillStyle
            ...
            Case [Gradient]
               Select Case SegmentNdx
                  Case 0, 3, 5, 6  ' these are the vertical digit segments
                     If Not ExponentFlag Then
                        BlitToRegion VirtualDC_SegV, hdc, m_SegmentWidth, m_SegmentHeight, LCD(SegmentNdx), StartX, StartY
                     Else
                        BlitToRegion VirtualDC_SegVExp, hdc, m_SegmentWidthExp, m_SegmentHeightExp, LCD(SegmentNdx), StartX, StartY
                     End If
                  Case 1, 2, 4, 7  ' these are the horizontal digit segments
                     If Not ExponentFlag Then
                        BlitToRegion VirtualDC_SegH, hdc, m_SegmentHeight, m_SegmentWidth, LCD(SegmentNdx), StartX, StartY
                     Else
                        BlitToRegion VirtualDC_SegHExp, hdc, m_SegmentHeightExp, m_SegmentWidthExp, LCD(SegmentNdx), StartX, StartY
                     End If
                  Case 8 ' decimal separator segment
                     BlitToRegion VirtualDC_Period, hdc, m_SegmentWidth, m_SegmentWidth, LCD(SegmentNdx), StartX, StartY
                  Case Else
                     FillRgn hdc, LCD(SegmentNdx), CurrentLitColorBrush
               End Select
            Case [Framed Gradient]
         End Select
      End If
   End If
...
'* this control to blit to the LED segment regions.  Thanks to LaVolpe   *
'* for his help in tweaking this routine.                                *
'*************************************************************************
...
Private Sub InitDigitalDisplay()
...
'  determine digital display background gradient that may
'  be used in the control, and also segment gradients.
   CalculateGradients
...
'     transfer the picture (with border) to the virtual DC bitmap.
      R = BitBlt(VirtualDC_BG, 0, 0, ScaleWidth, ScaleHeight, hdc, 0, 0, vbSrcCopy)
   Else
'     paint the gradient onto the virtual DC bitmap.
      Call StretchDIBits(VirtualDC_BG, _
                         0, 0, _
...
'     transfer the gradient in the virtual bitmap to the usercontrol.
      R = BitBlt(hdc, 0, 0, ScaleWidth, ScaleHeight, VirtualDC_BG, 0, 0, vbSrcCopy)
      CreateBorder
   End If
...
Private Sub CalculateSegmentGradients()
...
'  calculate the horizontal segment gradient for main value digits.
   CalculateGradient m_SegmentHeight, m_SegmentWidth + 1, _
                     TranslateColor(m_Seg3DColor1), TranslateColor(m_Seg3DColor2), _
                     90, m_Seg3DMiddleOut, _
                     SegHuBIH, SegHlBits()
   ...
   Call StretchDIBits(VirtualDC_SegH, _
                      ...
                      m_SegmentHeight, m_SegmentWidth, _
                      0, 1, _
                      m_SegmentHeight, m_SegmentWidth - 1, _
                      SegHlBits(0), SegHuBIH, _
                      DIB_RGB_COLORS, _
                      vbSrcCopy)

'  calculate the vertical segment gradient for main value digits.
   CalculateGradient m_SegmentWidth + 1, m_SegmentHeight, _
                     TranslateColor(m_Seg3DColor1), TranslateColor(m_Seg3DColor2), _
                     180, m_Seg3DMiddleOut, _
                     SegVuBIH, SegVlBits()

   Call StretchDIBits(VirtualDC_SegV, _
                      0, 0, _
                      m_SegmentWidth, m_SegmentHeight, _
                      1, 0, _
                      m_SegmentWidth - 1, m_SegmentHeight, _
                      SegVlBits(0), SegVuBIH, _
                      DIB_RGB_COLORS, _
                      vbSrcCopy)

'  calculate the horizontal segment gradient for exponent digits.
   CalculateGradient m_SegmentHeightExp, m_SegmentWidthExp + 1, _
                     TranslateColor(m_Seg3DColor1), TranslateColor(m_Seg3DColor2), _
                     90, m_Seg3DMiddleOut, _
                     SegHExpuBIH, SegHExplBits()

   Call StretchDIBits(VirtualDC_SegHExp, _
                      0, 0, _
                      m_SegmentHeightExp, m_SegmentWidthExp, _
                      0, 1, _
                      m_SegmentHeightExp, m_SegmentWidthExp - 1, _
                      SegHExplBits(0), SegHExpuBIH, _
                      DIB_RGB_COLORS, _
                      vbSrcCopy)

'  calculate the vertical segment gradient for exponent digits.
   CalculateGradient m_SegmentWidthExp + 1, m_SegmentHeightExp, _
                     TranslateColor(m_Seg3DColor1), TranslateColor(m_Seg3DColor2), _
                     180, m_Seg3DMiddleOut, _
                     SegVExpuBIH, SegVExplBits()

   Call StretchDIBits(VirtualDC_SegVExp, _
                      0, 0, _
                      m_SegmentWidthExp, m_SegmentHeightExp, _
                      1, 0, _
                      m_SegmentWidthExp - 1, m_SegmentHeightExp, _
                      SegVExplBits(0), SegVExpuBIH, _
                      DIB_RGB_COLORS, _
                      vbSrcCopy)

'  colon period virtual bitmap.
   Call StretchDIBits(VirtualDC_Period, _
                      0, 0, _
                      m_SegmentWidth, m_SegmentWidth, _
                      0, 0, _
                      m_SegmentWidth, m_SegmentWidth, _
                      SegHlBits(0), SegHuBIH, _
                      DIB_RGB_COLORS, _
...
'  create the control background virtual DC.
   CreateVirtualDC VirtualDC_BG, _
                   mMemoryBitmap_BG, mOriginalBitmap_BG, _
...
'  create the main value horizontal segment gradient virtual DC.
   CreateVirtualDC VirtualDC_SegH, _
                   ...
                   m_SegmentHeight + 1, m_SegmentWidth
...
'  create the main value vertical segment gradient virtual DC.
   CreateVirtualDC VirtualDC_SegV, _
                   ...
                   m_SegmentWidth, m_SegmentHeight + 10 '1

'  create the exponent horizontal segment gradient virtual DC.
   CreateVirtualDC VirtualDC_SegHExp, _
                   mMemoryBitmap_SegHExp, mOriginalBitmap_SegHExp, _
                   m_SegmentHeightExp, m_SegmentWidthExp

'  create the exponent vertical segment gradient virtual DC.
   CreateVirtualDC VirtualDC_SegVExp, _
                   mMemoryBitmap_SegVExp, mOriginalBitmap_SegVExp, _
                   m_SegmentWidthExp, m_SegmentHeightExp

'  create the colon period segment gradient virtual DC.
   CreateVirtualDC VirtualDC_Period, _
                   mMemoryBitmap_Period, mOriginalBitmap_Period, _
                   m_SegmentWidth - 1, m_SegmentWidth - 1
...
'*************************************************************************
   ...
   m_Seg3DAngle = m_def_Seg3DAngle
   m_Seg3DColor1 = m_def_Seg3DColor1
   m_Seg3DColor2 = m_def_Seg3DColor2
   m_Seg3DMiddleOut = m_def_Seg3DMiddleOut
...
'*************************************************************************
   ...
   With PropBag
      ...
      m_NumDigits = .ReadProperty("NumDigits", m_def_NumDigits)
      m_NumDigitsExp = .ReadProperty("NumDigitsExp", m_def_NumDigitsExp)
      m_PictureMode = .ReadProperty("PictureMode", m_def_PictureMode)
      m_Seg3DAngle = .ReadProperty("Seg3DAngle", m_def_Seg3DAngle)
      m_Seg3DColor1 = .ReadProperty("Seg3DColor1", m_def_Seg3DColor1)
      m_Seg3DColor2 = .ReadProperty("Seg3DColor2", m_def_Seg3DColor2)
      m_Seg3DMiddleOut = .ReadProperty("Seg3DMiddleOut", m_def_Seg3DMiddleOut)
...
'*************************************************************************
   ...
   With PropBag
      ...
      .WriteProperty "Seg3DAngle", m_Seg3DAngle, m_def_Seg3DAngle
      .WriteProperty "Seg3DColor1", m_Seg3DColor1, m_def_Seg3DColor1
      .WriteProperty "Seg3DColor2", m_Seg3DColor2, m_def_Seg3DColor2
      .WriteProperty "Seg3DMiddleOut", m_Seg3DMiddleOut, m_def_Seg3DMiddleOut
...
Public Property Get Seg3DAngle() As Single
   Seg3DAngle = m_Seg3DAngle
End Property

Public Property Let Seg3DAngle(ByVal New_Seg3DAngle As Single)
   m_Seg3DAngle = New_Seg3DAngle
   PropertyChanged "Seg3DAngle"
End Property

Public Property Get Seg3DColor1() As OLE_COLOR
   Seg3DColor1 = m_Seg3DColor1
End Property

Public Property Let Seg3DColor1(ByVal New_Seg3DColor1 As OLE_COLOR)
   m_Seg3DColor1 = New_Seg3DColor1
   PropertyChanged "Seg3DColor1"
   InitDigitalDisplay
   RedrawControl
End Property

Public Property Get Seg3DColor2() As OLE_COLOR
   Seg3DColor2 = m_Seg3DColor2
End Property

Public Property Let Seg3DColor2(ByVal New_Seg3DColor2 As OLE_COLOR)
   m_Seg3DColor2 = New_Seg3DColor2
   PropertyChanged "Seg3DColor2"
   InitDigitalDisplay
   RedrawControl
End Property

Public Property Get Seg3DMiddleOut() As Boolean
   Seg3DMiddleOut = m_Seg3DMiddleOut
End Property

Public Property Let Seg3DMiddleOut(ByVal New_Seg3DMiddleOut As Boolean)
   m_Seg3DMiddleOut = New_Seg3DMiddleOut
   PropertyChanged "Seg3DMiddleOut"
End Property
